...
## Fitting the Model + Tuning Cost Complexity

Next, we fit the model to the training folds to optimize the cost-complexity penalty. We will test 25 values to optimize the model performance.
...
```{r}
# fit rf model to CV folds w/ all levels for mtry
rf_fits <-
  rf_wkf |>
  tune_grid(resamples = water_folds,
            grid = 6,
...
rf_fits |>
  ...
  select(mtry, mean)
```
Checking out our parameter table, we see that our loss metric is minimized when there are `r rf_fits |> collect_metrics() |> arrange(mean) |> head(1) |> select(mtry) |> pull()` predictors considered at each split.

We'll save this value and then build the final model!
```{r}
rf_tuned_mtry <-
  rf_fits |>
  select_best(metric = "mn_log_loss")
...
rf_final_wkf <-
  rf_wkf |>
  finalize_workflow(rf_tuned_mtry)
```
...
```{r}
# final fit for classification tree
tree_final_fit <-
  tree_final_wkf |>
  last_fit(water_split,
           metrics = metric_set(accuracy, mn_log_loss))

# final fit for rf model
rf_final_fit <-
  rf_final_wkf |>
  last_fit(water_split,
           metrics = metric_set(accuracy, mn_log_loss))
...
# table to compare metrics
rbind(tree_metrics, rf_metrics) |>
  cbind(model =  rep(c("Tree", "Random Forest"), each = 2)) |>
  select(model, .metric, .estimate) |>
  rename("metric" = ".metric",
         "estimate" = ".estimate")
```
With a superior log loss and slightly superior accuracy, the **random forest model** did the best job at predicting water potability, correctly classifying `r 100 * (rf_metrics |> filter(.metric == "accuracy") |> pull(.estimate) |> round(4))`% of water sources!

We'll save this model so we can import it into the docker container
```{r}
